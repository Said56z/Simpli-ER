...
const RightSidebar = ({ setRegex }) => {
  ...
  return (
    <aside className="w-full lg:w-80 bg-accent-red lg:rounded-xl shadow-lg flex flex-col overflow-hidden h-fit mt-4 lg:mt-8">
          ...
          <div className="space-y-3">
            ...
            {ejemplosData.map((ex, i) => (
              <button 
                key={i} 
                onClick={() => setRegex(ex)}
                className="w-full bg-accent-red-dark/40 p-2 rounded text-center border border-white/10 hover:bg-white/20 transition-all active:scale-95 cursor-pointer block"
              >
                {ex}
              </button>
            ))}
